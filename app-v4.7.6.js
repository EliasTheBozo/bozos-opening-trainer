...
function bozoNameModerationReason(value='') {
  ...
  const compact = normalizeNameForModeration(raw);

  // Keep the user-facing error generic so the filter does not become an evasion guide.
  const blocked = [
    /f+u+c+k+/,
    /s+h+i+t+/,
    /b+i+t+c+h+/,
  ...
  return blocked.some(pattern => pattern.test(compact))
    ? 'That name is not allowed on BOZO.'
    : '';
}
