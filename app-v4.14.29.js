...
function reviewStructuredMoveAnalysis(row, selectedIndex) {
  ...
  // Recognize a near-term promotion only when the verified continuation actually shows it.
  const pvMoves=(row.principalVariationSan||[]).map(x=>String(x||''));
  const actualNext=(facts?.actualNextMoves||[]).map(x=>String(x||''));
  const futureMoves=[...pvMoves.slice(0,8),...actualNext.slice(0,8)];
  const promotionMove=futureMoves.find(san=>/=([QRBN])/.test(san) || /^[a-h](?:x[a-h])?[18][QRBN][+#]?$/.test(san));
  let promotionPlan=null;
  if(move.piece==='p' && promotionMove) {
    const promotionFile=String(promotionMove)[0];
    if(promotionFile===String(move.to||'')[0]) {
      const rank=Number(String(move.to||'')[1]);
      const distance=row.mover==='w' ? 8-rank : rank-1;
      if(distance>=0 && distance<=3) {
        promotionPlan={move:promotionMove,pawnSquare:move.to,distance};
        ...
        gameConnections.push(`The verified continuation shows this pawn reaching promotion with ${promotionMove}.`);
      }
    }
  }
...
function reviewStructureTakeaway(row, s) {
  ...
  if(s?.promotionPlan) {
    return `${row.san} is about promotion, not merely gaining space. Once a passed pawn gets this close, calculate whether the opponent can stop it before doing anything else.`;
  }

  if(move.piece==='p' && prep) {
    if (/^(Bb2|Bg2|Bb7|Bg7)$/.test(prep)) {
      return `After ${row.san}, look for ${prep} next: the pawn move is mainly about giving the bishop a useful development square.`;
    }
    return `${row.san} is preparation, not an end in itself. Remember the follow-up ${prep}.`;
  }
  if(move.piece==='b' && ['c1','f1','c8','f8'].includes(move.from)) {
    const longDiagonal=['b2','g2','b7','g7'].includes(move.to);
    return longDiagonal
      ? `${row.san} completes the development idea: get the bishop onto the long diagonal and let the earlier pawn move contribute to piece activity.`
      : `${row.san} is a development move: improve the bishop first, then judge what its new diagonal lets you do.`;
  }
  if(move.piece==='n' && ['b1','g1','b8','g8'].includes(move.from)) {
    return central.length
      ? `Develop the knight with a purpose: ${move.to} brings it into the game while influencing ${central.join(' and ')}.`
      : `Bring undeveloped knights into the game before spending extra tempi on pieces that are already active.`;
  }
  if(move.isCastle) return `When castling is safe and useful, one move can improve both king safety and rook coordination.`;

  const cmp=s?.bestMoveComparison;
  if(cmp && cmp.bestMove && !row.wasTop) {
    if(cmp.bestPurpose) return `When comparing candidate moves, ask which move solves more problems at once. Here ${cmp.bestMove} ${cmp.bestPurpose}.`;
    return `Do not stop after finding a playable move. Compare it with ${cmp.bestMove} and ask what extra job the more precise move accomplishes.`;
  }

  return String(s?.primaryIdea||`Remember what ${row.san} changes in the position.`).replace(/\.$/,'')+'.';
}
...
function reviewDeterministicTeachingFromStructure(row, s) {
  ...
  if(s.promotionPlan) {
    ...
    sentences.push(`${row.san} pushes the pawn closer to promotion.`);
    sentences.push(`The key point is concrete: the verified continuation reaches ${promo.move}, so the pawn's advance is the decisive feature of the position.`);
