...
async function loadPuzzleCloudData(mode=puzzleCloudMode){
  ...
  try{
    ...
    if(uid){
      ...
      jobs.push(sb.from('puzzle_runs').select('score,hints_used,avg_solve_time,ended_reason,duration_seconds,created_at').eq('user_id',uid).eq('mode',mode).order('created_at',{ascending:false}).limit(6));
    }
    ...
    if(history) history.innerHTML=myRuns.length?myRuns.map(row=>`<div class="puzzle-cloud-row"><span class="puzzle-cloud-rank">${puzzleCloudDate(row.created_at)}</span><span class="puzzle-cloud-player"><b>${Number(row.score||0)} solved</b><small>${escapeHtml(row.ended_reason||'complete')} · ${Number(row.hints_used||0)} hint${Number(row.hints_used||0)===1?'':'s'} used</small></span><span class="puzzle-cloud-score">${Number(row.avg_solve_time||0)>0?Number(row.avg_solve_time).toFixed(1)+'s':'—'}<small>avg</small></span></div>`).join(''):'<div class="empty-state mini"><span>No runs saved for this mode yet.</span></div>';
  }catch(error){
...
function puzzleHasFreeMajorCapture(fen) {
  ...
  for (const move of legal) {
    ...
    // A major piece can still be effectively free even when the capturing piece
    // can be recaptured. Example: Rxa4 Q? followed by ...Rxa4 still trades a
    // rook for a queen, a trivial +4 material pickup rather than a puzzle.
    // Judge the immediate exchange, not merely whether *some* recapture exists.
    let replies=[];
    ...
    // Reject any one-move major-piece pickup worth at least a minor piece after
    // the obvious recapture. This catches hanging queens taken by rooks/minors
    // as well as truly loose rooks. Equal queen trades and genuine exchanges
    // with no material windfall are allowed through to the deeper tactic gate.
    const trivialMajorWindfall=(move.captured==='q' || move.captured==='r') && immediateNetGain>=3;
    if (trivialMajorWindfall) {
      return {
        side,
        enemy,
        uci:`${move.from}${move.to}${move.promotion||''}`,
...
async function buildGeneralPuzzle() {
  ...
  for (let attempt=0; attempt<PUZZLE_MAX_GENERATION_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      const result={...spec,candidates,motif:quality.motif,solutionPv,targetUserMoves,quality:{gap:quality.gap,depth:PUZZLE_VERIFY_DEPTH,fenStateVerified:true}};
      bestFallback=result;
...
async function startNextPuzzle() {
  ...
  if (puzzleGeneralMode) {
    ...
    $('puzzle-subtitle').textContent = `${puzzleGeneralMotif} · ${puzzleUserSide[0].toUpperCase()+puzzleUserSide.slice(1)} to move`;
...
function finishPuzzleSession(reason='complete') {
  ...
  if (isPuzzleRunMode()) {
    ...
    savePuzzleRunCloud({score,hintsUsed:puzzleRunHintsUsed,avgSolve:avg,peakDifficulty:0,reason,durationSeconds}).catch(()=>{});
