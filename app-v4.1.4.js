...
function applyRatedMatchSnapshot(next) {
  ...
  ratedMatchSession = {...ratedMatchSession, ...next, game:chessFromRatedMoves(next.moves || []), myColor:next.white_id === state.session.user.id ? 'w':'b'};
  ratedMatchSelectedSquare = null;
  paintRatedOnlineGame();
  updateRatedOnlineStatus();
  if (next.status === 'completed') { $('bot-review-button').hidden=false; loadBozoRatings(); stopRatedMatchPolling(); }
}
...
function startRatedMatchPolling(){ stopRatedMatchPolling(); ratedMatchPollTimer=setInterval(refreshRatedMatchFromServer,1200); }
...
function paintRatedOnlineGame() {
  ...
  $$('[data-rated-square]').forEach(b=>{b.addEventListener('click',()=>handleRatedOnlineSquare(b.dataset.ratedSquare)); b.addEventListener('contextmenu',e=>e.preventDefault());});
  $('bot-user-arrow-layer').innerHTML=''; $('bot-move-list').innerHTML=renderDuelMoveRows(game.history());
}
async function handleRatedOnlineSquare(square) {
  if(!ratedMatchSession||ratedMatchSession.status!=='active')return;
  if(!ratedOnlineIsMyTurn()){toast('Waiting for your opponent.');return;}
  const game=ratedMatchSession.game,piece=game.get(square);
  if(!ratedMatchSelectedSquare){if(piece&&piece.color===ratedMatchSession.myColor){ratedMatchSelectedSquare=square;paintRatedOnlineGame();}return;}
  if(piece&&piece.color===ratedMatchSession.myColor){ratedMatchSelectedSquare=square;paintRatedOnlineGame();return;}
  const from=ratedMatchSelectedSquare; ratedMatchSelectedSquare=null;
  const candidate=game.moves({square:from,verbose:true}).find(m=>m.to===square);
  if(!candidate){paintRatedOnlineGame();toast('That move is not legal.');return;}
  $('bot-game-message').textContent='Sending move…'; const ok=await submitRatedOnlineMove(candidate); if(ok)await refreshRatedMatchFromServer(); else paintRatedOnlineGame();
}
async function submitRatedOnlineMove(move) {
  if(!ratedMatchSession?.id||!move)return false;
  try { const uci=`${move.from}${move.to}${move.promotion||''}`; const {data,error}=await sb.functions.invoke('rated-match',{body:{action:'move',matchId:ratedMatchSession.id,move:uci}}); if(error)throw error; if(data?.error)throw new Error(data.error); return true; }
  catch(error){console.warn('Rated move rejected:',error);toast(error?.message||'That online move could not be played.');await refreshRatedMatchFromServer();return false;}
}
