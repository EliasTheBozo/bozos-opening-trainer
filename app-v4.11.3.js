...
async function loadPuzzleCloudData(mode=puzzleCloudMode){
  ...
  try{
    ...
    if(uid){
      ...
      jobs.push(sb.from('puzzle_runs').select('score,hints_used,avg_solve_time,peak_difficulty,ended_reason,duration_seconds,created_at').eq('user_id',uid).eq('mode',mode).order('created_at',{ascending:false}).limit(6));
    }
...
async function savePuzzleRunCloud(payload){
  ...
  if(error){ console.warn('Could not sync puzzle run.',error); puzzleCloudSetStatus('Run finished, but cloud sync failed. Apply BOZO_V4112_PUZZLE_CLOUD_ACTIVITY.sql and try again.','error'); return null; }
...
async function startBozoPuzzles(mode='standard') {
  ...
  puzzleRunSolved = 0; puzzleRunStartedAt = 0; puzzleCurrentStartedAt = 0; puzzleSolveTimes = []; puzzleRunOutcomes = [];
...
// Reject generated positions whose "tactic" is polluted by a trivially
// hanging major piece.  This is deliberately evaluated from LEGAL captures
// in the root position (and legal immediate recaptures after the capture),
// rather than raw attack maps, so pinned/illegal defenders do not rescue a
// bad candidate.  A puzzle should test a concrete idea, not whether the
// player notices a queen or rook that can simply be taken for nothing.
function puzzleHasFreeMajorCapture(fen) {
  const game=new Chess(fen);
  ...
  for (const move of legal) {
    if (!move?.captured || !['q','r'].includes(move.captured)) continue;
    ...
    // If the opponent has no legal immediate recapture on the destination,
    // the major piece was genuinely loose. Queens are always disqualifying;
    // rooks are disqualifying when taken by an equal-or-cheaper piece.
    let replies=[];
    try { replies=after.moves({verbose:true})||[]; } catch (_) { replies=[]; }
    const canRecapture=replies.some(reply=>reply.to===move.to && Boolean(reply.captured));
    if (canRecapture) continue;

    const triviallyWinningQueen=move.captured==='q';
    const triviallyWinningRook=move.captured==='r' && moverValue<=capturedValue;
    if (triviallyWinningQueen || triviallyWinningRook) {
      return {
        side,
        enemy,
        uci:`${move.from}${move.to}${move.promotion||''}`,
...
async function verifyGeneralPuzzleCandidate(spec, initialLines) {
  ...
  const freeMajor=puzzleHasFreeMajorCapture(spec.fen);
  if (freeMajor) {
    console.debug('Puzzle candidate rejected: loose major piece', freeMajor);
    return null;
  }
...
async function buildGeneralPuzzle() {
  ...
  for (let attempt=0; attempt<PUZZLE_MAX_GENERATION_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      const solutionPv=(quality.lines[0]?.pv||[]).slice(0,20);
      const inspected=inspectPuzzlePv(spec.fen,quality.lines[0],20);
      const mateLine=quality.motif.key==='mate' || inspected.game?.in_checkmate?.();
      const targetUserMoves=mateLine ? Math.max(2,Math.min(8,Math.ceil(solutionPv.length/2))) : Math.max(2,Math.min(5,Math.ceil(solutionPv.length/2)));
      const difficulty=Math.round(700 + Math.min(1700, quality.gap*2.5 + inspected.moves.length*70 + inspected.checks*80 + inspected.captures*55 + (['sacrifice','mate','fork'].includes(quality.motif.key)?220:0)));
      const result={...spec,candidates,motif:quality.motif,solutionPv,targetUserMoves,difficulty,quality:{gap:quality.gap,depth:PUZZLE_VERIFY_DEPTH,fenStateVerified:true}};
      bestFallback=result;
...
async function startNextPuzzle() {
  ...
  if (puzzleGeneralMode) {
    ...
    puzzleCurrentDifficulty = spec.difficulty || 1200; puzzlePeakDifficulty=Math.max(puzzlePeakDifficulty,puzzleCurrentDifficulty); puzzleResolutionEvals=[]; puzzleFailedCurrent=false; puzzleCurrentStartedAt=Date.now();
    ...
    $('puzzle-subtitle').textContent = `${puzzleGeneralMotif} · ${puzzleUserSide[0].toUpperCase()+puzzleUserSide.slice(1)} to move · ~${puzzleCurrentDifficulty} difficulty`;
...
async function showPuzzleHint() {
  ...
  else if (/sacrifice/i.test(puzzleGeneralMotif)) clue='A temporary material investment may open the position.';
...
function updatePuzzleUI() {
  ...
  $('puzzle-instruction').textContent = puzzleGame?.game_over()
    ...
    : `${puzzleSolvedInCurrent}/${puzzleTargetUserMoves} continuation moves solved · difficulty ~${puzzleCurrentDifficulty||'—'}`;
...
function finishPuzzleSession(reason='complete') {
  ...
  if ($('puzzle-result-difficulty')) $('puzzle-result-difficulty').textContent=puzzlePeakDifficulty?`~${puzzlePeakDifficulty}`:'—';
