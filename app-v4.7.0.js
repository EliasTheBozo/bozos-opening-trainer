...
document.querySelectorAll('[data-postgame-close]').forEach(el=>el.addEventListener('click',closePostGameSummary));
...
$('postgame-review')?.addEventListener('click',()=>{const id=$('postgame-review').dataset.gameId;closePostGameSummary();if(id)openHistoricalGameReview(id);else route('review');});
$('postgame-rematch')?.addEventListener('click',async()=>{closePostGameSummary();if(ratedMatchSession){await requestRatedRematch();openRatedMatchModal();}else route('play');});
