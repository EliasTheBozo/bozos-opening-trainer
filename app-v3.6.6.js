...
function paintBoardUserAnnotations(boardId) {
  ...
  const markers = Object.entries(colors).map(([name,color]) => `
    <marker id="user-arrow-${safeId}-${name}" markerWidth="24" markerHeight="24" refX="22" refY="12" orient="auto" markerUnits="userSpaceOnUse">
      <path d="M1,1 L23,12 L1,23 L7,12 Z" fill="${color}"></path>
    </marker>`).join('');
  ...
  const markup = state.items.map(item => {
    ...
    if (item.type === 'arrow' && validSquare(item.from) && validSquare(item.to)) {
      ...
      const shortenEnd = (a, b, amount = 22) => {
      ...
      if (isKnightRoute) {
        ...
        const elbow = fileDelta === 2
          ? { x:to.x, y:from.y }
          : { x:from.x, y:to.y };
        const end = shortenEnd(elbow, to, 22);
        return `<path d="M ${from.x} ${from.y} L ${elbow.x} ${elbow.y} L ${end.x} ${end.y}" fill="none" stroke="${color}" stroke-width="13" stroke-linecap="square" stroke-linejoin="miter" opacity=".86" marker-end="url(#user-arrow-${safeId}-${colorName})"></path>`;
      }

      const end = shortenEnd(from, to, 22);
      return `<line x1="${from.x}" y1="${from.y}" x2="${end.x}" y2="${end.y}" stroke="${color}" stroke-width="13" stroke-linecap="square" opacity=".82" marker-end="url(#user-arrow-${safeId}-${colorName})"></line>`;
    }
    return '';
  }).join('');
