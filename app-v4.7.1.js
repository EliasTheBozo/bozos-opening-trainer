...
$('postgame-review')?.addEventListener('click', async () => {
  const button = $('postgame-review');
  ...
  try {
    ...
    if (game?.pgn) {
      route('review');
      setTimeout(() => {
        if ($('review-pgn-input')) $('review-pgn-input').value = game.pgn;
        if ($('review-import-message')) {
          $('review-import-message').textContent =
            `Rated game vs @${game.opponent_username || 'opponent'} loaded from your completed game.`;
        }
        $('review-pgn-input')?.scrollIntoView({ behavior:'smooth', block:'center' });
      }, 120);
    } else {
...
$('postgame-rematch')?.addEventListener('click', async () => {
  const button = $('postgame-rematch');
  if (button?.disabled) return;
  if (button) {
    button.disabled = true;
