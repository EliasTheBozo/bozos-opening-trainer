...
function renderShell() {
  ...
  $('owner-nav').hidden = !isOwner;
  $('owner-mobile-nav').hidden = !isOwner;
  $('dashboard-owner-shortcut').hidden = !isOwner;
  document.querySelector('.site-header')?.classList.toggle('owner-shell', isOwner);
  if ($('owner-nav')) {
    $('owner-nav').textContent = isOwner ? 'Owner' : "Owner's Office";
    $('owner-nav').title = "Owner's Office";
    $('owner-nav').setAttribute('aria-label', "Owner's Office");
  }
...
function openHistoricalGameReview(gameId) {
  ...
  setTimeout(() => {
    ...
    $('review-import-message').textContent = `Rated game vs @${game.opponent_username || 'opponent'} loaded from Game History.`;
...
async function openFriendProfile(username) {
  ...
  const [{ data, error }, { data: socialData, error: socialError }, chessResult] = await Promise.all([
...
function paintBoardUserAnnotations(boardId) {
  ...
  const markup = state.items.map(item => {
    ...
    if (item.type === 'square' && validSquare(item.square)) {
      ...
      return `<rect x="${center.x-48}" y="${center.y-48}" width="96" height="96" rx="7" fill="${color}" opacity=".27"></rect>`;
    }
...
async function computeWebsiteReview(sans, depth, bookDepth, onProgress) {
  ...
  for (let index = 0; index < plies.length; index++) {
    ...
    const isTerminal = game.isGameOver();
    ...
    if (isCheckmate) {
      ...
      if (isTerminal) terminal = { type: 'draw', winner: null };
    }
    ...
    const classification = isCheckmate
      ? { label: 'Best', cls: 'best' }
      : classifyReviewLoss(engineLoss, isBook);
...
async function initializeBozoRating(tier) {
  ...
  try {
    ...
    setTimeout(closeRatingSetup, 500);
  } catch (error) {
...
function applyRatedMatchSnapshot(next) {
  ...
  // While an optimistic move is awaiting server confirmation, an ordinary poll
  // can still return the older server row. Ignore that stale snapshot instead of
  // visually undoing the move the player just made.
  ...
  // Polling may return the exact same match state many times per minute. Do not
  // clear the user's selected piece unless the authoritative board actually changed.
  ...
  if (positionChanged) ratedMatchSelectedSquare = null;
...
async function refreshRatedMatchFromServer() {
  ...
  try {
    ...
    if(fresh) applyRatedMatchSnapshot(fresh);
  } catch(error) { console.warn('Rated match refresh failed:',error); }
...
function openRatedMatchModal() {
  ...
  $('bot-review-button').hidden=s.status!=='completed'; $('bot-eval-label').textContent='Rated'; $('bot-eval-white').style.width='50%';
  registerRatedOnlineBoardInput();
  paintRatedOnlineGame(); updateRatedOnlineStatus(); startRatedMatchPolling();
}
function subscribeRatedMatch(matchId) {
  if(ratedMatchChannel){try{sb.removeChannel(ratedMatchChannel)}catch(_){}}
  ratedMatchChannel=sb.channel(`rated-match-${matchId}`).on('postgres_changes',{event:'UPDATE',schema:'public',table:'live_rated_matches',filter:`id=eq.${matchId}`},payload=>applyRatedMatchSnapshot(payload.new)).subscribe(status=>{if(status==='SUBSCRIBED') refreshRatedMatchFromServer();});
}
function paintRatedOnlineGame() {
  ...
  $('web-bot-board').innerHTML=ranks.flatMap(rank=>files.map(file=>{const sq=`${file}${rank}`,sym=board[8-rank][file.charCodeAt(0)-97],c=[]; if(sq===ratedMatchSelectedSquare)c.push('bot-selected-square'); if(targets.includes(sq))c.push('bot-legal-square'); return `<button type="button" class="${c.join(' ')}" data-rated-square="${sq}">${webPiece(sym)}</button>`;})).join('');
...
async function handleRatedOnlineSquare(square) {
  ...
  // Optimistic UI: render the legal move immediately so rated play feels local,
  // while the server remains authoritative. A rejected move is rolled back.
  ratedMatchSelectedSquare = null;
  ...
  paintRatedOnlineGame();
  $('bot-game-message').textContent = 'Waiting for your opponent to move.';
  ...
  // Reconcile the optimistic board with the exact server acknowledgement.
  ratedMatchSession = {
    ...ratedMatchSession,
...
function updateRatedOnlineStatus(){if(!ratedMatchSession)return;$('bot-phase-label').textContent=ratedMatchSession.status==='completed'?'Finished':'Rated online';if(ratedMatchSession.status==='completed'){$('bot-turn-badge').textContent='Game complete';$('bot-game-message').textContent=ratedOnlineResultText();return;}const mine=ratedOnlineIsMyTurn();$('bot-turn-badge').textContent=mine?'Your move':'Opponent to move';$('bot-game-message').textContent=mine?'Your move.':'Waiting for your opponent to move.';}
...
function closeWebBotGame() {
  ...
  ratedMatchSession=null; ratedMatchSelectedSquare=null; ratedMatchSyncBusy=false;
...
function reviewWebBotGame() {
  const game=ratedMatchSession?.game||webBotSession?.game;if(!game)return;const wasRated=Boolean(ratedMatchSession),pgn=game.pgn();closeWebBotGame();route('review');setTimeout(()=>{$('review-pgn-input').value=pgn;$('review-import-message').textContent=wasRated?'Rated online game loaded. Choose the analysis settings and click Analyze game.':'BOZO Bot game loaded. Choose the analysis settings and click Analyze game.';$('review-pgn-input').scrollIntoView({behavior:'smooth',block:'center'});},100);
}
...
$('matchmaking-pool')?.addEventListener('change', updateMatchmakingRatingSummary);
...
const PUZZLE_MAX_GENERATION_ATTEMPTS = 30;
...
function setTrainMode(mode = 'recall') {
  trainMode = mode === 'bozo-puzzles' ? 'bozo-puzzles' : mode === 'puzzles' ? 'puzzles' : 'recall';
  ...
  if ($('train-new-line')) $('train-new-line').textContent = recall ? 'Choose another line' : puzzleGeneralMode ? 'New BOZO puzzle' : 'New puzzle line';
...
function resetCurrentTrainMode() {
  if (trainMode === 'puzzles' || trainMode === 'bozo-puzzles') {
    puzzleOpening = null;
    puzzlePool = [];
    puzzleGame = null;
    if (trainMode === 'bozo-puzzles') showBozoPuzzlePicker(); else showPuzzlePicker();
...
$('bozo-puzzle-start')?.addEventListener('click', startBozoPuzzles);
...
async function startBozoPuzzles() {
  route('train');
...
function weightedPuzzleMoveChoice(game, legal, ply) {
  const scoreMove = move => {
    let weight = 10;
    const piece = game.get(move.from);
    const fromRank = Number(move.from[1]);
    const toRank = Number(move.to[1]);
    if (move.captured) weight += 8 + puzzlePieceValue(move.captured) * 2;
    if (/[+#]/.test(move.san)) weight += 9;
    if (/O-O/.test(move.san)) weight += 14;
    if (move.promotion) weight += 20;
    if (piece?.type === 'p' && ['d','e'].includes(move.from[0]) && ply < 10) weight += 10;
    if ((piece?.type === 'n' || piece?.type === 'b') && ((piece.color==='w' && fromRank===1) || (piece.color==='b' && fromRank===8)) && ply < 16) weight += 9;
    if (piece?.type === 'q' && ply < 10) weight *= .2;
    if (piece?.type === 'k' && !/O-O/.test(move.san) && ply < 18) weight *= .08;
    if (piece?.type === 'r' && ply < 10) weight *= .35;
...
function generateBozoPosition() {
  // Reach a natural-looking legal position through weighted play. This is only a
  // candidate source; the engine/tactical filter below decides whether it is a puzzle.
  for (let attempt=0; attempt<50; attempt++) {
    const game = new Chess();
    const target = 16 + Math.floor(Math.random()*24);
    const history = [];
    for (let ply=0; ply<target && !game.game_over(); ply++) {
      const legal = game.moves({verbose:true});
      if (!legal.length) break;
      const chosen = weightedPuzzleMoveChoice(game, legal, ply);
    ...
    const whiteMaterial = pieces.filter(p=>p.color==='w' && p.type!=='k').reduce((s,p)=>s+puzzlePieceValue(p.type),0);
    const blackMaterial = pieces.filter(p=>p.color==='b' && p.type!=='k').reduce((s,p)=>s+puzzlePieceValue(p.type),0);
    if (nonKings < PUZZLE_MIN_NONKING_PIECES) continue;
    if (Math.abs(whiteMaterial-blackMaterial) > 8) continue;
...
function inspectPuzzlePv(fen, line, maxPlies=8) {
  ...
  return {rootColor,startBalance,endBalance:puzzleMaterialBalance(game,rootColor),materialSwing:puzzleMaterialBalance(game,rootColor)-startBalance,moves,checks,captures,promotions,game};
}

function detectPuzzleTacticalMotif(fen, line) {
  ...
  const pv=inspectPuzzlePv(fen,line,8);

  if (/#/.test(first.san) || pv.game.in_checkmate?.()) return {key:'mate',label:'Mating tactic',confidence:'verified'};
  if (first.promotion || pv.promotions) return {key:'promotion',label:'Promotion tactic',confidence:'verified'};

  // Reject the classic low-quality generator result: simply taking a loose piece
  // with no forcing follow-up. Captures can still qualify through the verified
  // combination/forcing tests below.
  ...
  const plainLoosePickup=Boolean(capturedPiece && capturedValue>=3 && firstTargetDefenders===0 && pv.checks===0 && pv.captures<=1);
  ...
  // A fork is only accepted when the PV demonstrates a concrete payoff: another
  // capture, a material swing, or a forcing check sequence after the fork move.
  if ((piecesHit.length>=2 || (kingHit && piecesHit.length>=1)) && (pv.captures>=1 || pv.materialSwing>=2 || pv.checks>=2))
    return {key:'fork',label:'Fork / double attack',confidence:'verified'};

  // A sacrifice must be visible in the continuation, not merely be an expensive
  // piece capturing something cheaper. Require the piece to be offered/taken and
  // compensation to appear as material recovery or forcing play.
  const offered = pv.moves[1]?.to===firstUci.slice(2,4) && pv.moves[1]?.capturedValue===movedValue;
  if (movedValue>=3 && offered && (pv.materialSwing>=-1 || pv.checks>=2 || pv.captures>=3))
    return {key:'sacrifice',label:'Calculated sacrifice',confidence:'verified'};
  if (capturedPiece && movedValue>capturedValue+1 && pv.captures>=3 && (pv.materialSwing>=-1 || pv.checks>=2))
    return {key:'sacrifice',label:'Material sacrifice',confidence:'verified'};
  ...
  const queenTrades=pv.moves.filter(m=>m.captured==='q').length;
  if (queenTrades && pv.captures>=2 && pv.moves.length>=3)
    return {key:'transition',label:'Tactical simplification / transition',confidence:'verified'};
...
async function verifyGeneralPuzzleCandidate(spec, initialLines) {
  if (!initialLines?.length) return null;
  const initialMotif=detectPuzzleTacticalMotif(spec.fen,initialLines[0]);
  ...
  const engine=await getReviewEngine();
  const verified=await engine.analyzeMultiPv(spec.fen,PUZZLE_VERIFY_DEPTH,6);
  if (!verified.length) return null;
  const motif=detectPuzzleTacticalMotif(spec.fen,verified[0]);
  if (!motif || motif.key!==initialMotif.key) return null;
  const pv=inspectPuzzlePv(spec.fen,verified[0],8);
  const gap=puzzleTacticalGap(verified);
  // For quieter tactical transitions/forcing sequences, insist that choosing the
  // right idea matters. Direct mates, promotions, sacrifices and forks are concrete
  // enough to stand on their own even when more than one move works.
...
async function buildGeneralPuzzle() {
  // Two-pass quality gate: discover at a moderate depth, then independently verify
  // the motif at a deeper depth before the position is allowed into a session.
  let bestFallback=null;
  for (let attempt=0; attempt<PUZZLE_MAX_GENERATION_ATTEMPTS; attempt++) {
    const spec=generateBozoPosition();
    if (!spec) continue;
    puzzleGame=new Chess(spec.fen); puzzleUserSide=spec.userSide;
    puzzleCandidateFen=''; puzzleCandidateMoves=[];
    try {
      const engine=await getReviewEngine();
      const discovery=await engine.analyzeMultiPv(spec.fen,PUZZLE_GENERATION_DEPTH,6);
      const quality=await verifyGeneralPuzzleCandidate(spec,discovery);
      if (!quality) continue;
      // Build accepted alternatives from the deeper verification pass so the puzzle
      // and the move judge agree about what counts as a playable branch.
      ...
      const result={...spec,candidates,motif:quality.motif,quality:{gap:quality.gap,depth:PUZZLE_VERIFY_DEPTH,fenStateVerified:true}};
      bestFallback=result;
      if (candidates.length>=2 || ['sacrifice','fork','mate','promotion'].includes(quality.motif.key)) return result;
    } catch(error) { console.warn('Puzzle candidate rejected by quality gate.',error); }
  }
  return bestFallback;
}
...
async function startNextPuzzle() {
  ...
  if (puzzleGeneralMode) {
    ...
    puzzleTargetUserMoves = 3; puzzleSolvedInCurrent = 0; puzzleSelectedSquare = null;
    ...
    $('puzzle-subtitle').textContent = `${puzzleGeneralMotif} · ${puzzleUserSide[0].toUpperCase()+puzzleUserSide.slice(1)} to move · ${puzzleCandidateMoves.length} suggested path${puzzleCandidateMoves.length===1?'':'s'}`;
...
/* BOZO v4.2.0 — dockable music player */
const BOZO_MUSIC_DOCK_KEY = 'bozo_music_dock_v1';

function findBozoMusicPlayer() {
  const selectors = [
    '#music-player',
    '#spotify-player',
    '.music-player',
  ...
  // Fallback for the current BOZO player: find the floating element that
  // contains the Spotify label and transport controls.
  const candidates = [...document.querySelectorAll('body *')];
  return candidates.find(el => {
    if (!(el instanceof HTMLElement)) return false;
    const text = (el.textContent || '').trim();
    if (!/spotify/i.test(text)) return false;
...
function loadBozoMusicDockState() {
  try {
    return {
      dock: 'bottom-right',
      collapsed: false,
      hidden: false,
      ...JSON.parse(localStorage.getItem(BOZO_MUSIC_DOCK_KEY) || '{}')
    };
  } catch {
    return { dock: 'bottom-right', collapsed: false, hidden: false };
  }
}

function saveBozoMusicDockState(next) {
  try { localStorage.setItem(BOZO_MUSIC_DOCK_KEY, JSON.stringify(next)); } catch {}
}

let bozoMusicDockState = loadBozoMusicDockState();
let bozoMusicDragging = false;
let bozoMusicDragPointer = null;
let bozoMusicDragOffset = { x: 0, y: 0 };

function ensureBozoMusicDockTools(player) {
  if (!player) return;

  player.dataset.musicDockReady = '1';

  let tools = player.querySelector('.music-dock-tools');
  if (!tools) {
    tools = document.createElement('div');
    tools.className = 'music-dock-tools';
    tools.setAttribute('aria-label', 'Music player controls');
    tools.innerHTML = `
      <button id="music-player-minimize" class="music-dock-tool" type="button" title="Minimize music player" aria-label="Minimize music player">−</button>
      <button id="music-player-dismiss" class="music-dock-tool" type="button" title="Hide music player" aria-label="Hide music player">×</button>
    `;
    player.prepend(tools);
  }

  if (!player.querySelector('.music-drag-handle')) {
    const handle = document.createElement('button');
    handle.className = 'music-drag-handle';
    handle.type = 'button';
    handle.title = 'Drag to move music player';
    handle.setAttribute('aria-label', 'Move music player');
    handle.innerHTML = '<span></span><span></span><span></span>';
    player.prepend(handle);
  }

  const minimize = player.querySelector('#music-player-minimize');
  const dismiss = player.querySelector('#music-player-dismiss');
  const handle = player.querySelector('.music-drag-handle');

  if (minimize && !minimize.dataset.bound) {
    minimize.dataset.bound = '1';
    minimize.addEventListener('click', event => {
      event.stopPropagation();
      bozoMusicDockState.collapsed = !bozoMusicDockState.collapsed;
      saveBozoMusicDockState(bozoMusicDockState);
      applyBozoMusicDockState();
    });
  }
  ...
  if (dismiss && !dismiss.dataset.bound) {
    ...
    dismiss.addEventListener('click', event => {
      event.stopPropagation();
      bozoMusicDockState.hidden = true;
      saveBozoMusicDockState(bozoMusicDockState);
      applyBozoMusicDockState();
    });
  }
  ...
  if (handle && !handle.dataset.bound) {
    ...
    handle.addEventListener('pointerdown', event => {
      ...
      bozoMusicDragging = true;
      bozoMusicDragPointer = event.pointerId;
      bozoMusicDragOffset = {
      ...
      player.classList.add('music-dragging');
    ...
    handle.addEventListener('pointermove', event => {
      if (!bozoMusicDragging || event.pointerId !== bozoMusicDragPointer) return;

      const rect = player.getBoundingClientRect();
      const maxX = Math.max(8, window.innerWidth - rect.width - 8);
      const maxY = Math.max(72, window.innerHeight - rect.height - 8);
      const x = Math.min(maxX, Math.max(8, event.clientX - bozoMusicDragOffset.x));
      const y = Math.min(maxY, Math.max(72, event.clientY - bozoMusicDragOffset.y));
    ...
    const finishDrag = event => {
      if (!bozoMusicDragging || event.pointerId !== bozoMusicDragPointer) return;
      bozoMusicDragging = false;
      bozoMusicDragPointer = null;
      player.classList.remove('music-dragging');
      ...
      const distances = [
        ['left', cx],
        ['right', vw - cx],
        ['bottom-left', Math.hypot(cx, vh - cy)],
        ['bottom-right', Math.hypot(vw - cx, vh - cy)]
      ].sort((a,b) => a[1] - b[1]);

      bozoMusicDockState.dock = distances[0][0];
      saveBozoMusicDockState(bozoMusicDockState);
      applyBozoMusicDockState();
    };
...
function applyBozoMusicDockState() {
  const player = findBozoMusicPlayer();
  if (!player) return;

  ensureBozoMusicDockTools(player);

  player.classList.remove(
    'music-dock-bottom-right',
    'music-dock-bottom-left',
    'music-dock-left',
    'music-dock-right',
    'music-collapsed',
    'music-hidden'
  ...
  const dock = compactViewport && ['left','right'].includes(bozoMusicDockState.dock)
    ? 'bottom-right'
    : bozoMusicDockState.dock;

  player.classList.add(`music-dock-${dock}`);

  if (bozoMusicDockState.collapsed) player.classList.add('music-collapsed');
  if (bozoMusicDockState.hidden) player.classList.add('music-hidden');

  const minimize = player.querySelector('#music-player-minimize');
  if (minimize) {
    minimize.textContent = bozoMusicDockState.collapsed ? '+' : '−';
    minimize.title = bozoMusicDockState.collapsed ? 'Expand music player' : 'Minimize music player';
  }
}

function restoreBozoMusicPlayer() {
  const player = findBozoMusicPlayer();
  if (!player) return;
  bozoMusicDockState.hidden = false;
  saveBozoMusicDockState(bozoMusicDockState);
  applyBozoMusicDockState();
}

// Make the existing navbar Music control also restore the player when dismissed.
document.addEventListener('click', event => {
  const trigger = event.target.closest('button,a');
  if (!trigger) return;
  const text = (trigger.textContent || '').trim();
  if (/^music$/i.test(text) || /spotify/i.test(trigger.getAttribute('aria-label') || '')) {
    if (bozoMusicDockState.hidden) {
      bozoMusicDockState.hidden = false;
      saveBozoMusicDockState(bozoMusicDockState);
      setTimeout(applyBozoMusicDockState, 0);
    }
  }
});

window.addEventListener('resize', () => {
  if (!bozoMusicDragging) applyBozoMusicDockState();
});

document.addEventListener('DOMContentLoaded', () => {
  setTimeout(applyBozoMusicDockState, 100);
});

// In case the player is created after login/Spotify initialization.
const bozoMusicDockObserver = new MutationObserver(() => {
  const player = findBozoMusicPlayer();
  if (player && player.dataset.musicDockReady !== '1') {
    applyBozoMusicDockState();
  }
});
bozoMusicDockObserver.observe(document.documentElement, { childList: true, subtree: true });

window.restoreBozoMusicPlayer = restoreBozoMusicPlayer;
