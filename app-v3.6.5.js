...
function paintBoardUserAnnotations(boardId) {
  ...
  const markers = Object.entries(colors).map(([name,color]) => `
    <marker id="user-arrow-${safeId}-${name}" markerWidth="18" markerHeight="18" refX="16" refY="9" orient="auto" markerUnits="userSpaceOnUse">
      <path d="M1,1 L17,9 L1,17 L5.5,9 Z" fill="${color}"></path>
    </marker>`).join('');
  ...
  const markup = state.items.map(item => {
    ...
    if (item.type === 'arrow' && validSquare(item.from) && validSquare(item.to)) {
      ...
      const shortenEnd = (a, b, amount = 17) => {
      ...
      if (isKnightRoute) {
        ...
        const elbow = fileDelta === 2
          ? { x:to.x, y:from.y }
          : { x:from.x, y:to.y };
        const end = shortenEnd(elbow, to, 17);
        return `<path d="M ${from.x} ${from.y} L ${elbow.x} ${elbow.y} L ${end.x} ${end.y}" fill="none" stroke="${color}" stroke-width="8" stroke-linecap="round" stroke-linejoin="round" opacity=".86" marker-end="url(#user-arrow-${safeId}-${colorName})"></path>`;
      }

      const end = shortenEnd(from, to, 17);
      return `<line x1="${from.x}" y1="${from.y}" x2="${end.x}" y2="${end.y}" stroke="${color}" stroke-width="8" stroke-linecap="round" opacity=".86" marker-end="url(#user-arrow-${safeId}-${colorName})"></line>`;
    }
    return '';
  }).join('');
