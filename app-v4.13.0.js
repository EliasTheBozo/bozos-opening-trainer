...
$('friend-profile-message')?.addEventListener('click',async()=>{const u=$('friend-profile-challenge')?.dataset.username;if(!u||!commsSignedIn())return;closeFriendProfile();const r=await sb.rpc('bozo_get_or_create_dm',{target_username:u});if(r.error)return toast(readableError(r.error));bozoCommsThread={id:r.data,type:'dm',title:'@'+u};await openComms('messages');bozoCommsThread={id:r.data,type:'dm',title:'@'+u};renderComms();});
