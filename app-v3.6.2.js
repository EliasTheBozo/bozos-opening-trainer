...
function generateBozoPosition() {
  // Generate a legal, non-book-dependent position by playing weighted-random legal moves.
  // Avoid the first phase of the game so these are primarily middlegame/endgame decisions.
  for (let attempt=0; attempt<40; attempt++) {
    const game = new Chess();
    const target = 18 + Math.floor(Math.random()*28);
    const history = [];
    for (let ply=0; ply<target && !game.game_over(); ply++) {
      const legal = game.moves({verbose:true});
      if (!legal.length) break;
      // Prefer captures/checks occasionally so generated positions are less sterile.
      const forcing = legal.filter(m => m.captured || /[+#]/.test(m.san));
      const pool = forcing.length && Math.random() < .32 ? forcing : legal;
    ...
    if (nonKings < 5) continue;
    return { fen:game.fen(), history, userSide:game.turn()==='b'?'black':'white' };
  }
  return { fen:new Chess().fen(), history:[], userSide:'white' };
}

async function buildGeneralPuzzle() {
  // Try several generated positions and favor ones with at least two engine-approved paths.
  let fallback = null;
...
async function startNextPuzzle() {
  ...
  if (puzzleGeneralMode) {
    ...
    $('puzzle-subtitle').textContent = `Generated position · ${puzzleUserSide[0].toUpperCase()+puzzleUserSide.slice(1)} to move · ${puzzleCandidateMoves.length} engine-approved path${puzzleCandidateMoves.length===1?'':'s'}`;
    $('puzzle-number').textContent = `${puzzleStats.index+1}/${puzzleStats.total}`;
    $('puzzle-start-label').textContent = 'generated position';
    setPuzzleFeedback('neutral','Choose your path.', puzzleCandidateMoves.length > 1 ? `${puzzleCandidateMoves.length} moves are currently playable. Find one you believe in.` : 'Find a strong move. BOZO judges the move by its actual evaluation, not a preset answer.');
