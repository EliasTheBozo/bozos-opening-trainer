...
let dailyEditorSetupMode=false,dailySetupPieces={},dailySetupTool='wP';
...
function paintDailyEditorBoard(){const board=$('daily-editor-board');if(!board||(!dailyEditorGame&&!dailyEditorSetupMode))return; board.classList.toggle('setup-mode',dailyEditorSetupMode); board.innerHTML=[0,1,2,3,4,5,6,7].flatMap(r=>[0,1,2,3,4,5,6,7].map(f=>{const sq=dailySquareName(f,r),p=dailyEditorPieceAt(sq);return `<button type="button" class="board-square ${(r+f)%2?'dark':'light'}${dailyEditorSelected===sq?' selected':''}" data-daily-editor-square="${sq}" aria-label="${sq}">${dailyPieceMarkup(p)}</button>`})).join('');board.querySelectorAll('[data-daily-editor-square]').forEach(b=>{b.addEventListener('click',()=>dailyEditorClick(b.dataset.dailyEditorSquare));b.addEventListener('contextmenu',e=>{if(!dailyEditorSetupMode)return;e.preventDefault();delete dailySetupPieces[b.dataset.dailyEditorSquare];paintDailyEditorBoard();});});}
function dailyEditorClick(sq){
  if(dailyEditorSetupMode){
    if(dailySetupTool==='erase') delete dailySetupPieces[sq]; else {const c=dailySetupTool[0],t=dailySetupTool[1].toLowerCase(); dailySetupPieces[sq]={color:c,type:t};}
    paintDailyEditorBoard();return;
  }
  const p=dailyEditorGame.get(sq);if(!dailyEditorSelected){if(p&&p.color===dailyEditorGame.turn()){dailyEditorSelected=sq;paintDailyEditorBoard()}return;}if(p&&p.color===dailyEditorGame.turn()){dailyEditorSelected=sq;paintDailyEditorBoard();return;}const from=dailyEditorSelected;dailyEditorSelected=null;let m=null;try{m=dailyEditorGame.move({from,to:sq,promotion:'q'})}catch{}if(m){dailyEditorMoves.push(m.from+m.to+(m.promotion||''));paintDailyEditorLine()}paintDailyEditorBoard();
}
