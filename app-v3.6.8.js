...
function paintBoardUserAnnotations(boardId) {
  ...
  const markers = Object.entries(colors).map(([name,color]) => `
    <marker id="user-arrow-${safeId}-${name}"
            ...
            viewBox="0 0 30 30"
            overflow="visible">
      <path d="M0 0 L30 15 L0 30 Z" fill="${color}"></path>
    </marker>`).join('');
  ...
  const markup = state.items.map(item => {
    ...
    if (item.type === 'arrow' && validSquare(item.from) && validSquare(item.to)) {
      ...
      const shortenEnd = (a, b, amount = 14) => {
      ...
      if (isKnightRoute) {
        ...
        const elbow = fileDelta === 2
          ? { x:to.x, y:from.y }
          : { x:from.x, y:to.y };
        const end = shortenEnd(elbow, to, 14);
        return `<path d="M ${from.x} ${from.y} L ${elbow.x} ${elbow.y} L ${end.x} ${end.y}" fill="none" stroke="${color}" stroke-width="18" stroke-linecap="square" stroke-linejoin="miter" opacity=".82" marker-end="url(#user-arrow-${safeId}-${colorName})"></path>`;
      }

      const end = shortenEnd(from, to, 14);
      return `<line x1="${from.x}" y1="${from.y}" x2="${end.x}" y2="${end.y}" stroke="${color}" stroke-width="18" stroke-linecap="square" opacity=".82" marker-end="url(#user-arrow-${safeId}-${colorName})"></line>`;
    }
    return '';
  }).join('');
