...
async function handleRatedOnlineSquare(square) {
  ...
  // Freeze the current selection while the authoritative server validates the move.
  ratedMatchSelectedSquare = null;
  paintRatedOnlineGame();
  $('bot-game-message').textContent = `Sending ${candidate.san || `${from}-${square}`}…`;
  ...
  // The Edge Function has accepted this exact move. Reflect that authoritative
  // acknowledgement immediately instead of waiting for a potentially-busy poll.
  const uci = `${candidate.from}${candidate.to}${candidate.promotion || ''}`;
  const nextMoves = [...(Array.isArray(ratedMatchSession.moves) ? ratedMatchSession.moves : []), uci];
  ...
  // Then reconcile against Supabase. Realtime and the normal polling fallback
  // continue to synchronize the opponent's browser independently.
  setTimeout(() => refreshRatedMatchFromServer(), 80);
}
