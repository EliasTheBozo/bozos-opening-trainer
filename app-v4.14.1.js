  ...
  const moreMenu = ({ openingId, labelSource, compact = false }) => `
    <details class="opening-more-menu ${compact ? 'compact' : ''}">
      ...
      <div class="opening-more-popover">
        ...
        <button class="opening-master-button" onclick="openMasterGamesForOpening('${escapeHtml(labelSource).replace(/'/g, "\\'")}')">
          <span class="more-action-title">Master games</span>
          <small>Study real games in this opening.</small>
        </button>
...
function openMasterGamesForOpening(name){openMasterGames({query:name});}
...
const masterState={games:[],current:null,orientation:'white',ply:0,mode:'study',game:null,selected:null,exact:0,different:0,decisions:[],startedAt:null,locked:false};
...
async function loadMasterGames(){
  ...
  const {data,error}=await sb.rpc('search_master_games',{p_query:q||null,p_year:year?Number(year):null,p_result:result||null,p_limit:80});
  if(error){list.innerHTML=`<div class="empty-state"><div>♜</div><b>Master Database needs setup</b><span>${escapeHtml(readableError(error))}</span></div>`;return;}
  masterState.games=data||[];$('master-count').textContent=String(masterState.games.length);renderMasterGameList();
...
function renderMasterGameList(){
  ...
  list.innerHTML=masterState.games.map(g=>`<button class="master-game-row ${masterState.current?.id===g.id?'active':''}" data-master-id="${g.id}" type="button"><strong>${escapeHtml(g.white||'White')} <em>${escapeHtml(g.result||'*')}</em> ${escapeHtml(g.black||'Black')}</strong><span>${escapeHtml(masterGameSub(g))}</span></button>`).join('');
...
async function selectMasterGame(id){
  ...
  $('master-event').textContent=game.event||'MASTER GAME';$('master-white').textContent=game.white||'White';$('master-black').textContent=game.black||'Black';$('master-result').textContent=game.result||'*';$('master-opening').textContent=[game.eco,game.opening].filter(Boolean).join(' · ');$('master-details').textContent=[game.white_elo?`${game.white_elo}`:'',game.black_elo?`${game.black_elo}`:'',masterDisplayDate(game.game_date),game.site].filter(Boolean).join(' · ');
  document.querySelectorAll('[data-master-mode]').forEach(x=>x.classList.toggle('active',x.dataset.masterMode==='study'));
  masterSetStudyPly(0);renderMasterGameList();
}
...
function masterSetStudyPly(ply){if(!masterState.current)return;const max=masterMovesSan().length;masterState.ply=Math.max(0,Math.min(max,Number(ply)||0));masterState.game=masterBuildGame(masterState.ply);masterState.selected=null;paintMasterBoard();paintMasterMoves();paintMasterPly();}
...
function masterBoardClick(sq){
  ...
  else{masterState.different++;masterState.locked=true;$('master-training-feedback').textContent=`Playable choice recorded. The game continuation was ${masterSan}.`;paintMasterBoard();setTimeout(()=>{masterState.game.undo();try{masterState.game.move(masterSan,{sloppy:true});}catch{}masterState.ply++;masterState.locked=false;paintMasterBoard();paintMasterMoves();paintMasterPly();masterAutoAdvanceTraining();},720);}
...
function masterAutoAdvanceTraining(){
  ...
  paintMasterBoard();paintMasterMoves();paintMasterPly();if(masterState.ply>=moves.length)return finishMasterTraining();
