...
function compactIdentityBadges(profile={}, role='') {
  ...
  if (profile.country_code) bits.push(`<span class="live-country-flag" title="${escapeHtml(profile.country_code)}">${countryFlag(profile.country_code)}</span>`);
...
function renderProfile() {
  ...
  if (titleRow) {
    ...
    const flag = p.country_code ? `<span class="profile-country-chip" title="${escapeHtml(p.country_code)}">${countryFlag(p.country_code)}</span>` : '';
  ...
  $('profile-flair-input').value = p.selected_flair || '';
...
$('profile-save-button').addEventListener('click', async () => {
  ...
  const { error } = await sb.from('profiles').update({
    ...
    selected_flair: selectedFlair || null,
