...
function paintBoardUserAnnotations(boardId) {
  ...
  const markers = Object.entries(colors).map(([name,color]) => `
    <marker id="user-arrow-${safeId}-${name}" markerWidth="28" markerHeight="28" refX="26" refY="14" orient="auto" markerUnits="userSpaceOnUse">
      <path d="M1,1 L27,14 L1,27 L8.5,14 Z" fill="${color}"></path>
    </marker>`).join('');
  ...
  const markup = state.items.map(item => {
    ...
    if (item.type === 'square' && validSquare(item.square)) {
      ...
      return `<rect x="${center.x-47}" y="${center.y-47}" width="94" height="94" rx="7" fill="${color}" opacity=".27"></rect>`;
    }
    if (item.type === 'arrow' && validSquare(item.from) && validSquare(item.to)) {
      ...
      const shortenEnd = (a, b, amount = 25) => {
        const dx = b.x-a.x, dy = b.y-a.y;
        const length = Math.hypot(dx,dy) || 1;
        return { x:b.x-(dx/length)*amount, y:b.y-(dy/length)*amount };
      };

      if (isKnightRoute) {
        // Draw a true orthogonal L. Travel the two-square component first,
        // then turn once toward the destination.
        const elbow = fileDelta === 2
          ? { x:to.x, y:from.y }
          : { x:from.x, y:to.y };
        const end = shortenEnd(elbow, to, 25);
        return `<path d="M ${from.x} ${from.y} L ${elbow.x} ${elbow.y} L ${end.x} ${end.y}" fill="none" stroke="${color}" stroke-width="17" stroke-linecap="square" stroke-linejoin="miter" opacity=".86" marker-end="url(#user-arrow-${safeId}-${colorName})"></path>`;
      }

      const end = shortenEnd(from, to, 25);
      return `<line x1="${from.x}" y1="${from.y}" x2="${end.x}" y2="${end.y}" stroke="${color}" stroke-width="17" stroke-linecap="square" opacity=".80" marker-end="url(#user-arrow-${safeId}-${colorName})"></line>`;
    }
    return '';
  }).join('');
...
const PUZZLE_BRANCH_DEPTH = 11;
const PUZZLE_PLAYABLE_CP_WINDOW = 120;
const PUZZLE_GENERATION_DEPTH = 10;
const PUZZLE_MIN_NONKING_PIECES = 10;
const PUZZLE_MAX_GENERATION_ATTEMPTS = 22;
...
function generateBozoPosition() {
  ...
  for (let attempt=0; attempt<50; attempt++) {
    ...
    for (let ply=0; ply<target && !game.game_over(); ply++) {
      ...
      game.move(chosen.san); history.push(chosen.san);
    }
    ...
    return { fen:game.fen(), history, userSide:game.turn()==='b'?'black':'white' };
  }
  return null;
}
...
function detectPuzzleTacticalMotif(fen, line) {
  ...
  const movingPiece=game.get(firstUci.slice(0,2));
  const capturedPiece=game.get(firstUci.slice(2,4));
  const first=puzzleMoveFromUci(game, firstUci);
  if (!first) return null;
  const beforeMaterial=puzzleMaterialTotal(game);
  const movedValue=puzzlePieceValue(movingPiece?.type);
  const capturedValue=puzzlePieceValue(capturedPiece?.type);
  game.move({from:firstUci.slice(0,2),to:firstUci.slice(2,4),promotion:firstUci[4]||'q'});

  if (/#/.test(first.san)) return { key:'mate', label:'Mating tactic' };
  if (first.promotion) return { key:'promotion', label:'Promotion tactic' };

  const movedAfter=game.get(firstUci.slice(2,4));
  const enemy= movingPiece?.color==='w' ? 'b':'w';
  const attacked = puzzleAttacksFrom(game, firstUci.slice(2,4), movedAfter)
    .map(sq=>({sq,p:game.get(sq)})).filter(x=>x.p && x.p.color===enemy && x.p.type!=='p');
  const valuableTargets=attacked.filter(x=>puzzlePieceValue(x.p.type)>=3);
  const attacksKing=valuableTargets.some(x=>x.p.type==='k');
  const nonKingTargets=valuableTargets.filter(x=>x.p.type!=='k');
  if (nonKingTargets.length>=2 || (attacksKing && nonKingTargets.length>=1)) return { key:'fork', label:'Fork / double attack' };

  if (capturedPiece && movedValue > capturedValue + 1) return { key:'sacrifice', label:'Material sacrifice' };

  // A non-capturing move that is immediately taken in the principal variation is
  // also a sacrifice candidate, provided the sacrificed unit is a real piece.
  if (!capturedPiece && movedValue>=3 && line.pv[1]) {
    const reply=line.pv[1];
    if (reply.slice(2,4)===firstUci.slice(2,4)) return { key:'sacrifice', label:'Piece sacrifice' };
  }

  // Follow the first few principal-variation plies and look for forcing play or a
  // genuine material transition. Plain one-move hanging-piece pickups do not qualify.
  let forcingCount = /[+#]/.test(first.san) ? 1 : 0;
  let captureCount = capturedPiece ? 1 : 0;
  let queenBefore = game.board().flat().filter(p=>p?.type==='q').length + (capturedPiece?.type==='q'?1:0);
  const pvGame = new Chess(fen);
  for (const uci of line.pv.slice(0,6)) {
    const m=puzzleMoveFromUci(pvGame,uci); if(!m) break;
    if (/[+#]/.test(m.san)) forcingCount++;
    if (m.captured) captureCount++;
    pvGame.move({from:uci.slice(0,2),to:uci.slice(2,4),promotion:uci[4]||'q'});
    if (pvGame.game_over()) break;
  }
  const queenAfter=pvGame.board().flat().filter(p=>p?.type==='q').length;
  const materialDrop=beforeMaterial-puzzleMaterialTotal(pvGame);
  if (forcingCount>=2 && (captureCount>=1 || forcingCount>=3)) return { key:'forcing', label:'Forcing tactical sequence' };
  if (queenBefore>queenAfter && materialDrop>=8) return { key:'transition', label:'Tactical simplification / transition' };
  if (materialDrop>=8 && captureCount>=3) return { key:'combination', label:'Multi-move combination' };
  return null;
}

async function buildGeneralPuzzle() {
  // Generate candidates until we find a position whose principal variation contains
  // an actual tactical idea. A plain free-piece pickup is deliberately not enough.
  ...
  for (let attempt=0; attempt<PUZZLE_MAX_GENERATION_ATTEMPTS; attempt++) {
    ...
    try {
      const engine=await getReviewEngine();
      const lines=await engine.analyzeMultiPv(spec.fen, PUZZLE_GENERATION_DEPTH, 6);
      if (!lines.length) continue;
      const motif=detectPuzzleTacticalMotif(spec.fen, lines[0]);
      if (!motif) continue;
      const candidates=await loadPuzzleCandidates();
      const result={...spec,candidates,motif};
      bestFallback=result;
      // Branching is preferred, but tactical quality is more important than forcing
      // every puzzle to have several equal moves.
      if (candidates.length>=2 || ['sacrifice','fork','mate','promotion'].includes(motif.key)) return result;
    } catch(error) { console.warn('Puzzle candidate rejected.',error); }
  }
  return bestFallback;
}
