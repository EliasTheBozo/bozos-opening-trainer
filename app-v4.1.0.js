...
function closeRatingSetup() {
  if ($('rating-setup-modal')) $('rating-setup-modal').hidden = true;
}
...
function openRatedMatchModal() {
  // Reuse the existing BOZO bot modal board shell for the first online-play
  // surface, but disable bot behavior and render the server-backed match.
  if (!ratedMatchSession) return;

  if (!$('bot-game-modal')) {
    alert('Match found, but the game board could not open.');
    return;
  }
  ...
  webBotSession = {
    ...
    bookSans: [],
    requiredBookPlies: 0,
    playerColor: s.myColor,
    strengthKey: '',
    strength: { label: 'Online opponent' },
    phase: 'online',
  ...
  $('bot-game-title').textContent = 'Rated Online Game';
  $('bot-game-subtitle').textContent = `${formatPoolName(s.pool)} · ${Math.round(s.base_seconds/60)}+${s.increment_seconds}`;
  $('bot-book-name').textContent = s.opponent_username || 'Online opponent';
  $('bot-book-pgn').textContent = `${s.my_display_rating || '?'} vs ${s.opponent_display_rating || '?'}`;
  $('bot-player-color-label').textContent = s.myColor === 'w' ? 'White' : 'Black';
  $('bot-strength-label').textContent = 'Rated';
...
function subscribeRatedMatch(matchId) {
  if (ratedMatchChannel) {
    try { sb.removeChannel(ratedMatchChannel); } catch (_) {}
  }
  ratedMatchChannel = sb.channel(`rated-match-${matchId}`)
    .on('postgres_changes', {
      event: 'UPDATE',
      schema: 'public',
      table: 'live_rated_matches',
      filter: `id=eq.${matchId}`
    }, payload => {
      const next = payload.new;
      if (!ratedMatchSession || next.id !== ratedMatchSession.id) return;
      ratedMatchSession = { ...ratedMatchSession, ...next, game: chessFromRatedMoves(next.moves || []) };
      if (webBotSession?.onlineRated && webBotSession.matchId === next.id) {
        webBotSession.game = ratedMatchSession.game;
        webBotSession.status = next.status;
        webBotSession.moves = Array.isArray(next.moves) ? next.moves : [];
        if (next.status === 'completed') {
          webBotSession.resultReason = next.result === '1-0'
            ? 'White wins.'
            : next.result === '0-1'
              ? 'Black wins.'
              : 'Draw.';
...
async function submitRatedOnlineMove(move) {
  if (!webBotSession?.onlineRated || !webBotSession.matchId || !move) return false;
  try {
    const uci = `${move.from}${move.to}${move.promotion || ''}`;
    const { data, error } = await sb.functions.invoke('rated-match', {
      body: {
        action: 'move',
        matchId: webBotSession.matchId,
        move: uci
      }
    });
    if (error) {
      let message = error.message || 'Move rejected.';
      try {
        const context = await error.context?.json?.();
        if (context?.error) message = context.error;
      } catch (_) {}
    ...
    console.warn('Rated move rejected:', error);
    toast(error?.message || 'That online move could not be played.');
    return false;
  }
}
...
function handleWebBotSquare(square) {
  ...
  if (webBotSession.onlineRated) {
    // The browser verifies the move for immediate UX, but the server is authoritative.
    // Do not mutate the local game here; Realtime paints the server-confirmed state.
    submitRatedOnlineMove(candidate);
    paintWebBotGame();
    updateWebBotStatus();
    return;
  }
...
function updateWebBotStatus() {
  ...
  $('bot-turn-badge').textContent = playerTurn ? 'Your move' : (session.onlineRated ? 'Opponent to move' : 'BOZO Bot');
  if (playerTurn) {
...
async function resignWebBotGame() {
  ...
  if (webBotSession.onlineRated && webBotSession.matchId) {
    try {
      const { data, error } = await sb.functions.invoke('rated-match', {
        body: { action: 'resign', matchId: webBotSession.matchId }
      });
      if (error) throw error;
      if (data?.error) throw new Error(data.error);
      return;
    } catch (error) {
      toast(error?.message || 'Could not resign this online game.');
      return;
    }
  }
...
$('close-rating-setup')?.addEventListener('click', closeRatingSetup);
$('rating-setup-modal')?.addEventListener('click', event => {
  if (event.target === $('rating-setup-modal')) closeRatingSetup();
});
document.querySelectorAll('[data-rating-tier]').forEach(button => {
  button.addEventListener('click', () => initializeBozoRating(button.dataset.ratingTier));
});
