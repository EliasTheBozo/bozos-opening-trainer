...
function unlockDailySolved(){ $('daily-explanation-card').hidden=false; $('daily-discussion').hidden=false; $('daily-analyze').hidden=false; $('daily-explanation').textContent=dailyPuzzle.explanation||'Puzzle complete.'; $('daily-solution-line').textContent=(dailyPuzzle.main_line_san||dailyLine.join(' ')); loadDailyComments(); }
...
// Daily Puzzle Studio
let dailyEditorGame=null,dailyEditorMoves=[],dailyEditorSelected=null,dailyStudioMonth=dailyToday().slice(0,7);
...
async function loadDailyStudioDate(date){const {data}=await sb.rpc('get_daily_puzzle_editor',{p_date:date});const r=Array.isArray(data)?data[0]:data;resetDailyEditor();$('daily-editor-date').value=date;if(!r)return;$('daily-editor-title').value=r.title||'';$('daily-editor-theme').value=r.theme||'';$('daily-editor-fen').value=r.fen||'';$('daily-editor-status').value=r.status||'draft';$('daily-editor-hint1').value=r.hint1||'';$('daily-editor-hint2').value=r.hint2||'';$('daily-editor-hint3').value=r.hint3||'';$('daily-editor-explanation').value=r.explanation||'';$('daily-editor-alternates').value=(r.accepted_lines||[]).map(x=>x.join(' ')).join('\n');dailyEditorMoves=r.main_line_uci||[];try{dailyEditorGame=new Chess(r.fen);for(const u of dailyEditorMoves){const m=dailyEditorGame.moves({verbose:true}).find(x=>(x.from+x.to+(x.promotion||''))===u);if(m)dailyEditorGame.move(m)}}catch{}paintDailyEditorBoard();paintDailyEditorLine();}
