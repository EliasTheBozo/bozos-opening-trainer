...
async function loadMasterGames(){
  ...
  const {data,error}=await sb.rpc('search_master_games',{p_query:q||null,p_year:year?Number(year):null,p_result:result||null,p_limit:200});
...
function masterRevealViewerHeader(){
  const viewer=$('master-active')?.closest('.master-viewer'), header=document.querySelector('.site-header');
  if(!viewer||!header)return;
  requestAnimationFrame(()=>{
    const viewerTop=viewer.getBoundingClientRect().top;
    const headerBottom=header.getBoundingClientRect().bottom;
    if(viewerTop<headerBottom+12)viewer.scrollIntoView({block:'start',behavior:'smooth'});
  });
}
