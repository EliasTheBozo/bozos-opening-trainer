...
let suppressMasterAutoLoad = false;
...
function route(name) {
  ...
  if (name === 'masters' && !suppressMasterAutoLoad) loadMasterGames();
...
function renderOpeningFamily(family) {
  ...
  // v4.14.14: the visual preview may intentionally be a rich/deep variation,
  // but the FAMILY Master Games button should use the family's defining line.
  // Prefer an exact base row such as "Bird Opening" (1.f4); if none exists,
  // use the shortest published line as the safest family anchor.
  const masterAnchor = single
    ? preview
    : (
        family.lines.find(line =>
          String(line.name || '').trim().toLowerCase() ===
          String(family.name || '').trim().toLowerCase()
        )
        || [...family.lines].sort((a,b) =>
          a.moveCount - b.moveCount ||
          (a.pgn?.length || 0) - (b.pgn?.length || 0) ||
          String(a.displayVariation || '').localeCompare(String(b.displayVariation || ''))
        )[0]
        || preview
      );
    ...
    <article class="opening-family-card ${single ? 'single-line-family' : ''}">
      ...
      <div class="opening-action-dock ${single ? 'single' : 'family'}">
        <div class="opening-primary-actions">
          ...
          <button class="opening-master-direct" onclick="openMasterGamesForOpening('${masterAnchor.id}','${escapeHtml(single ? challengeName : family.name).replace(/'/g, "\\'")}','${single ? 'exact' : 'family'}')"><span class="master-games-button-icon" aria-hidden="true">♜</span><span>Master games</span></button>
          ${single ? '' : `
              ...
              <div class="family-line-row">
                ...
                <div class="line-content">
                  ...
                  <div class="line-action-row line-action-dock">
                    ...
                    <button class="line-master-button" onclick="openMasterGamesForOpening('${line.id}','${escapeHtml(lineChallengeName).replace(/'/g, "\\'")}','exact')">♜ Master games</button>
...
async function renderCommsThread(thread){
  const body=$('comms-body');
  const table=thread.type==='dm'?'bozo_dm_messages':'bozo_support_messages';
  const fk='thread_id';

  // v4.14.16: opening a DM is the read action. Persist it before refreshing
  // the badge so the orange unread count does not reappear on the next poll.
  if(thread.type==='dm'){
    const mark=await sb.rpc('bozo_mark_dm_thread_read',{p_thread_id:thread.id});
    if(mark.error)console.warn('Could not mark DM thread read',mark.error);
    await refreshCommsBadges();
  }
  ...
  if(error){
    body.innerHTML=commsEmpty('Could not open conversation',readableError(error));
    return;
  }

  body.innerHTML=`<button class="comms-thread" id="comms-thread-back">← Back</button><h3>${escapeHtml(thread.title||'Conversation')}</h3><div class="comms-messages">${(data||[]).map(m=>`<div class="comms-bubble ${m.sender_id===state.session.user.id?'mine':''}">${escapeHtml(m.body||'')}<small>${noticeTime(m.created_at)}</small></div>`).join('')||'<p>No messages yet.</p>'}</div><div class="comms-compose"><textarea id="comms-reply" rows="3" maxlength="4000" placeholder="Write a reply…"></textarea><button id="comms-send" class="button primary">Send</button></div><div class="comms-toolbar">${thread.type==='dm'?'<button id="comms-block">Block user</button><button id="comms-report" class="comms-report">Report conversation</button>':''}</div>`;

  $('comms-thread-back').onclick=()=>{
    bozoCommsThread=null;
    renderComms();
  };

  $('comms-send').onclick=async()=>{
    const text=$('comms-reply').value.trim();
    if(!text)return;
    ...
    if(e)return toast(readableError(e));
    $('comms-reply').value='';
    renderCommsThread(thread);
    refreshCommsBadges();
  };

  if(thread.type==='dm'){
    $('comms-block').onclick=()=>blockCurrentDm(thread.id);
    $('comms-report').onclick=()=>reportCurrentDm(thread.id);
  }
}
...
async function openMasterGamesForOpening(openingId,name='',scope='exact'){
  const label=String(name||'').trim();
  const matchScope=scope==='family'?'family':'exact';

  // Opening Library -> Master games should mean games classified under that
  // opening, not merely games that once transposed through the same position.
  suppressMasterAutoLoad=true;
  try{
    route('masters');
  }finally{
    suppressMasterAutoLoad=false;
  }

  if($('master-search')) $('master-search').value=label;
  if($('master-result-filter')) $('master-result-filter').value='';
  if($('master-year-filter')) $('master-year-filter').value='';

  return loadMasterGamesByOpening(label,matchScope);
}
window.openMasterGamesForOpening=openMasterGamesForOpening;

// BOZO v4.14.16 — Opening a DM persists read state and clears unread badge
// BOZO v4.14.15 — Opening Library Master Games matches PGN opening classification
// BOZO v4.14.14 — Family Master Games uses the defining/base opening line
// BOZO v4.14.13 — Canonical en-passant FEN keys for opening-position matching
...
const masterState={games:[],current:null,orientation:'white',ply:0,mode:'study',game:null,selected:null,exact:0,different:0,decisions:[],startedAt:null,locked:false,total:0,offset:0,pageSize:100,loading:false,hasMore:true,queryKey:'',openingContext:null};
...
async function loadMasterGames(options={}){
  ...
  if(append && masterState.openingContext){
    return loadMasterGamesByOpening(
      masterState.openingContext.name,
      masterState.openingContext.scope,
      {append:true}
    );
  }

  if(!append) masterState.openingContext=null;
...
function masterFenKey(fen){
  const parts=String(fen||'').trim().split(/\s+/);
  if(parts.length<4)return parts.slice(0,4).join(' ');

  let [placement,turn,castling,ep]=parts;

  if(ep && ep!=='-'){
    try{
      const probe=new Chess(fen);
      const legalEp=probe.moves({verbose:true}).some(move=>{
        const flags=String(move?.flags||'');
        return flags.includes('e');
      });
      if(!legalEp)ep='-';
    }catch(_){
      // If the FEN cannot be reloaded, keep the original field rather than
      // silently inventing a different position.
    }
  }

  return [placement,turn,castling||'-',ep||'-'].join(' ');
}
...
async function loadMasterGamesByOpening(openingName,scope='exact',options={}){
  const list=$('master-game-list'); if(!list)return;

  const append=!!options.append;
  const name=String(openingName||'').trim();
  const family=scope==='family';

  if(!name){
    masterState.openingContext=null;
    return loadMasterGames();
  }

  if(masterState.loading)return;

  const year=masterQueryValue('master-year-filter');
  const result=masterQueryValue('master-result-filter');
  const queryKey=JSON.stringify(['opening',name,family,year,result]);

  if(!append || queryKey!==masterState.queryKey){
    masterState.queryKey=queryKey;
    masterState.offset=0;
    masterState.total=0;
    masterState.hasMore=true;
    masterState.games=[];
    masterState.openingContext={name,scope:family?'family':'exact'};
    list.innerHTML='<div class="empty-state"><div>♟</div><b>Finding master games from this opening…</b></div>';
  }
  ...
  try{
    ...
    const jobs=[
      sb.rpc('search_master_games_by_opening_page',{
        ...args,
        p_limit:masterState.pageSize,
        p_offset:masterState.offset
      })
    ];

    if(!append) jobs.push(sb.rpc('count_master_games_by_opening',args));
    ...
    if($('master-count')) $('master-count').textContent=String(masterState.total);
    renderMasterGameList();
    ...
    if(note){
      ...
      note.textContent=family
        ? `${masterState.total} master game${masterState.total===1?'':'s'} classified as ${name} or one of its variations.`
        : `${masterState.total} master game${masterState.total===1?'':'s'} classified as ${name}.`;
    }

    if(!append && masterState.games.length){
      await selectMasterGame(masterState.games[0].id);
    }
  }catch(error){
    console.warn('Opening-classification master search failed',error);
    if(!append){
      masterState.games=[];
      masterState.total=0;
      masterState.hasMore=false;
      if($('master-count')) $('master-count').textContent='0';
      list.innerHTML=`<div class="empty-state"><div>⚠</div><b>Could not load games for this opening</b><span>${escapeHtml(readableError(error))}</span></div>`;
    }
  }finally{
...
async function loadMasterGamesByPosition(fenKey,label='Opening'){
  ...
  // v4.14.12: fetch a page of games and the true total independently.
