...
function paintWebClubs() {
  ...
  out.innerHTML = webClubs.map(c => `
    <article class="club-card">
      <div class="club-card-top">
        <div class="club-avatar">${escapeHtml((c.name || 'B').slice(0,1).toUpperCase())}</div>
  ...
  out.querySelectorAll('[data-club-join]').forEach(b => b.addEventListener('click', async () => {
    const { error } = await sb.rpc('bozo_join_club', { target_club_id: b.dataset.clubJoin });
    if (error) return toast(readableError(error));
    toast('Joined club.');
    clubFilter = 'mine';
    await loadWebClubs();
  ...
  out.querySelectorAll('[data-club-open]').forEach(b => b.addEventListener('click', () => {
    toast('Club page foundation is ready; detailed club pages come next.');
  }));
}
...
$('close-create-club')?.addEventListener('click', () => $('create-club-modal').hidden = true);
$('submit-create-club')?.addEventListener('click', async () => {
  ...
  const { error } = await sb.rpc('bozo_create_club', { club_name:name, club_slug:slug, club_description:description, club_visibility:visibility });
...
async function loadArenas() {
  ...
  if (state.session) await sb.rpc('bozo_ensure_official_arenas').catch(() => {});
...
function paintArenas() {
  ...
  out.innerHTML = arenaRows.map(a => {
    ...
    return `<article class="arena-card ${a.is_official ? 'official' : ''}">
      ...
      <button class="button ${live ? 'primary':'secondary'} small" data-arena-join="${a.id}">${a.joined ? 'Joined' : live ? 'Join arena' : 'Register'}</button>
    </article>`;
  }).join('');
...
$('close-create-arena')?.addEventListener('click', () => $('create-arena-modal').hidden = true);
