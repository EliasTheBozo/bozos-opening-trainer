...
function renderBozoPlusPage() {
  ...
  if (!state.session) {
    ...
    resetBozoPayPalCheckout('Sign in to subscribe with PayPal Sandbox.');
    return;
  }
...
async function getPublicCosmetics(username) {
  ...
  const { data, error } = await sb.rpc('get_public_cosmetics', { target_username: username });
...
async function paintRatedSupporterBadges() {
  ...
  const opponentUsername = ratedMatchSession.opponent_username;
  try {
    const opponent = await getPublicCosmetics(opponentUsername);
    if (!ratedMatchSession) return;
    if (opp) opp.hidden = !opponent?.is_supporter;
    const oppLabel = $('bot-strength-label');
    if (oppLabel && opponent?.is_supporter) oppLabel.style.color = validBozoNameColor(opponent.name_color);
    const myLabel = $('bot-player-color-label');
    if (myLabel && isBozoSupporter()) myLabel.style.color = validBozoNameColor(state.profile?.name_color);
  } catch (_) {}
}
...
async function ownerSearchUsers() {
  ...
  out.innerHTML = (data || []).map(u => `
    ...
    <p>${Number(u.xp||0).toLocaleString()} XP · ${Number(u.opening_mastery||0).toLocaleString()} mastery · ${u.is_suspended?'Suspended':'Active'}</p>
...
async function loadBozoPlusCheckout() {
  if (!state.session?.user) return resetBozoPayPalCheckout('Sign in to subscribe with PayPal Sandbox.');

  const message=$('bozo-paypal-checkout-message');
  if(message) message.textContent='Loading PayPal Sandbox checkout…';
  ...
  try {
    ...
    $('bozo-paypal-mode-pill').textContent=data.environment === 'sandbox' ? 'Sandbox · Fake money' : 'Live';
...
async function renderBozoPayPalButton(paypalSdk,containerId,planKey,planId) {
  ...
  await paypalSdk.Buttons({
    ...
    onCancel() {
      const message=$('bozo-paypal-checkout-message');
      if(message) message.textContent='PayPal checkout was cancelled. Nothing was charged.';
    },
    onError(error) {
...
async function initializeBozoPayPalSandbox() {
  ...
  try{
    ...
    toast('PayPal Sandbox backend is ready');
    await loadBozoPlusCheckout();
  }catch(error){output.textContent=readableError(error);}
  finally{button.disabled=false;button.textContent='Finish PayPal Sandbox setup';}
}
$('bozo-paypal-initialize')?.addEventListener('click',initializeBozoPayPalSandbox);
